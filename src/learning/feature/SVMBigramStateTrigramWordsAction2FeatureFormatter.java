...
public class SVMBigramStateTrigramWordsAction2FeatureFormatter extends SVMDictionaryBasedFeatureFormatter{
	public static String[] prevStateForSingle = null;
	public static int previousAction = 0;
	public static int previousAction2 = 0;
	
	public SVMBigramStateTrigramWordsAction2FeatureFormatter(Dictionary dictionary) {
		super(dictionary);
		System.out.println("Formatter: SVMBigramStateTrigramWordsAction2FeatureFormatter");
	...
	public void format(String fileName, String extension) throws Exception{
		...
		while((line = reader.readLine()) != null){
			...
			for(int i = 0; i < 4; i++){
				if(i == prevAction2)
					input += Integer.toString(dictionary.size() + 1 + 2 * (stateInput.length - 1) + 8 + 2 + 4 + i) + ":1 ";
			}
	...
	public String formatSingleInstance(String instruction, String environment) {
		...
		for(int i = 0; i < 4; i++){
			if(i == previousAction2)
				formatted += Integer.toString(dictionary.size() + 1 + 2 * (stateInput.length - 1) + 8 + 2 + 4 + i) + ":1 ";
		}
