...
public class ActionDefiner {
	...
	public static ArrayList<CellPair> getStraightPaths(ArrayList<Cell> path){
		...
		try {
			Direction prev = ActionDefiner.getDirection(start, end);
			Direction curr;
			int length = 2;
			int startIndex = 0;
			
			for(int i = 2; i < path.size(); i++){
				curr = ActionDefiner.getDirection(end, path.get(i));
				if(prev != curr){
					if(length > 2)
			...
			if(length > 2)
				paths.add(new CellPair(start, end, startIndex, path.size() - 1));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static int numberOfTurnRight(Agent agent, Direction direction){
		int agentDirection = agent.getDirection().ordinal();
		
		int total = 0;
		
		while(agentDirection != direction.ordinal()){
			if(agentDirection == 3)
				agentDirection = 0;
			else
				agentDirection++;
	...
	public static int numberOfTurnLeft(Agent agent, Direction direction){
		int agentDirection = agent.getDirection().ordinal();
